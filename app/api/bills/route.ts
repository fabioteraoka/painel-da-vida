...
export async function PATCH(request: Request) {
  const session = await auth();
  if (!session?.user?.email) return NextResponse.json({ error: "Não autenticado." }, { status: 401 });
  const body = await request.json() as { id?:string; paymentAccountId?:string|null; status?:"NEEDS_REVIEW"|"CONFIRMED"|"SCHEDULED"|"PAID"|"IGNORED" };
  ...
  const updated=await prisma.bill.update({
    where:{id:bill.id},
    data:{paymentAccountId:body.paymentAccountId===undefined?undefined:body.paymentAccountId,status:body.status??undefined},
    include:{paymentAccount:true}
  });
  return NextResponse.json({id:updated.id,paymentAccount:updated.paymentAccount?{id:updated.paymentAccount.id,name:updated.paymentAccount.name,type:updated.paymentAccount.type}:null,status:updated.status});
}
