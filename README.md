...
Configure `CRON_SECRET` na Vercel. O agendamento em `vercel.json` executa a coleta a cada 30 minutos; essa frequência exige Vercel Pro ou Enterprise. O plano Hobby só aceita cron diário e rejeitará este deploy. O primeiro deploy com o novo schema também precisa aplicar `npx prisma db push` ou uma migração Prisma equivalente antes de chamar essas rotas.
